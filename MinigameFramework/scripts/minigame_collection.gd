...
@onready var manager: MinigameManager = $MinigameManager
...
func _ready():
	...
	manager.all_minigames_completed.connect(_on_all_minigames_completed)
...
func _on_play_all_button_pressed():
	...
	manager.start(_minigame_data, true)
